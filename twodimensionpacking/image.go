...
func DrawBox(box *Box, imgName string, scale int) error {
	...
	dc.SetRGB(0, 0, 0)
	dc.Stroke()
	dc.SavePNG(imgName + ".png")
	return nil
}

func DrawBoxes(boxes []*Box, imgName string, scale int) {
	var wg sync.WaitGroup
	wg.Add(len(boxes))
	for i := range boxes {
		go func(i int) {
			err := DrawBox(boxes[i], imgName+fmt.Sprint(i+1), scale)
			if err != nil {
				log.Println(err)
			}
			wg.Done()
